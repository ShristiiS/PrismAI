...
WORD_DIR = DATASET_DIR / "generated" / "word"
...
def _extract_sprint_planning_blueprint(full_md: str, sprint_number: int) -> str:
    start_tag = f"## SPRINT {sprint_number} PLANNING DOCUMENT"
    end_tag = f"## SPRINT {sprint_number} RETROSPECTIVE"
    ...
    if end == -1:
        raise GenerationError(
            f"Could not find {end_tag!r} (end marker) in Sprint_Planning_and_Ret.md"
        )
    return full_md[start:end].strip()


def _sprint_out_path(sprint_number: int) -> Path:
    return WORD_DIR / f"sprint{sprint_number}_planning.docx"
...
def _validate_planning_spec(data: dict[str, Any]) -> None:
    ...
    if not isinstance(tables, list) or len(tables) < 2:
        raise ValueError("Need tables array with at least Team Capacity and Sprint Backlog")
    tc = tables[0]
    bl = tables[1]
    ...
    rows_tc = tc.get("rows") or []
    rows_bl = bl.get("rows") or []
    if not rows_tc:
    ...
    for r in rows_tc:
        if len(r) != 4:
            raise ValueError("Each team capacity row must have 4 cells")
    for r in rows_bl:
        if len(r) != 6:
            raise ValueError("Each sprint backlog row must have 6 cells")
    sc = data.get("success_criteria") or []
    dep = data.get("dependencies") or []
    risks = data.get("risks") or []
    dod = data.get("definition_of_done") or []
    if not sc or not isinstance(sc, list):
        raise ValueError("success_criteria must be a non-empty array of strings")
    if not dep or not isinstance(dep, list):
        raise ValueError("dependencies must be a non-empty array of strings")
    ...
    if not dod or not isinstance(dod, list):
        raise ValueError("definition_of_done must be a non-empty array of strings")
...
def _write_docx(data: dict[str, Any], out_path: Path, *, sprint_number: int) -> Path:
    doc = Document()
    ...
    for item in data.get("risks") or []:
        ...
        line = f"Risk: {risk} Mitigation: {mit}"
    ...
    try:
        ...
        fb = out_path.with_name(f"sprint{sprint_number}_planning_ready.docx")
        shutil.move(str(tmp), str(fb))
        return fb


def generate_sprint_planning_doc(sprint_number: int) -> Path:
    md_full = SOURCE_MARKDOWN.read_text(encoding="utf-8")
    blueprint = _extract_sprint_planning_blueprint(md_full, sprint_number)
    out_path = _sprint_out_path(sprint_number)

    user = (
        f"Return ONLY valid JSON (no markdown, no code fences) for a Sprint {sprint_number} planning Word document. "
        ...
        "{\n"
        f'  "title": "string (e.g. Sprint {sprint_number} Planning — Nutrivana)",\n'
    ...
    if saved != out_path:
        print(
            f"Sprint {sprint_number}: saved to {saved} (could not overwrite {out_path} — file may be open in Word). "
            f"Close sprint{sprint_number}_planning.docx and copy/rename, or rerun."
        )
    else:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate Nutrivana sprint planning Word documents from blueprints.")
    parser.add_argument(
        "sprints",
        nargs="*",
        type=int,
        ...
        metavar="N",
        help="Sprint numbers to generate (default: 1 through 9). Example: python generate_sprint_planning.py 1 2",
    )
    args = parser.parse_args(argv)
    sprints = args.sprints if args.sprints else list(range(1, 10))
    for n in sprints:
        if n < 1 or n > 9:
    ...
    print(f"Model: {MODEL}")
    for n in sprints:
        generate_sprint_planning_doc(n)
    return 0
