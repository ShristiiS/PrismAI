...
# User referenced "sprint_10-13_Meeting_Note.md"; in this repo the file is Sprint_10-13_Meeting_Notes.md.
SOURCE_MARKDOWN = DATASET_DIR / "Sprint_10-13_Meeting_Notes.md"
WORD_OUT_DIR = DATASET_DIR / "generated" / "word"
OUT_PATH = WORD_OUT_DIR / "sprint13_meeting_notes.docx"
...
def _extract_sprint13_meeting_blueprint(full_md: str) -> str:
    start_tag = "# MEETING NOTE 4 — SPRINT 13"
    start = full_md.find(start_tag)
    if start == -1:
        raise GenerationError(f"Could not find {start_tag!r} in {SOURCE_MARKDOWN.name}")
    return full_md[start:].strip()
...
def _call_meeting_json(client: OpenAI, *, blueprint: str) -> dict[str, Any]:
    user = (
        "Return ONLY valid JSON (no markdown, no code fences) for the Sprint 13 weekly meeting notes Word document.\n"
        "Follow the SYSTEM prompt: live-capture dialogue, ticket-level sprint health, no generic summaries.\n"
        "Key discussion topics must align to the blueprint: "
        "(1) prenatal presets launch results + BUG-014 post mortem (87s setup, all 11 users updated, folate 600mg vs 600mcg bug, fixed in 4 hours, testing coverage gap), "
        "(2) A/B test final results + 100% rollout decision (84.7% vs 61.3% completion, 47.3% vs 43.1% Day 7 retention, 135 more returning users per 1000 acquired), "
        "(3) Q2 OKR review preview (MAU 3512, NPS 41, retention 47% vs 50% target, 315% growth zero paid, Arjun's invisible improvements insight).\n\n"
        "JSON schema:\n"
        "{\n"
        '  "document_title": "string (match blueprint document name tone)",\n'
        '  "meeting_date": "string (should be June 27, 2025)",\n'
        '  "sprint_line": "string (should read Sprint 13 (June 17–June 30, 2025) if present in blueprint)",\n'
        '  "attendees": "comma-separated names",\n'
        '  "sprint_health_check": "string multi-paragraph narrative",\n'
        '  "key_discussion_1": {"title": "Prenatal presets launch results and BUG-014 post mortem", "notes": "long live notes with who said what"},\n'
        '  "key_discussion_2": {"title": "A/B test final results and 100% rollout decision", "notes": "..."},\n'
        '  "key_discussion_3": {"title": "Q2 OKR review preview", "notes": "..."},\n'
...
def _write_docx(spec: dict[str, Any], out_path: Path) -> Path:
    ...
    try:
        ...
        fb = out_path.with_name("sprint13_meeting_notes_ready.docx")
...
def main() -> int:
    ...
    blueprint = _extract_sprint13_meeting_blueprint(md)

    print(f"Model: {MODEL}")
    print("Calling API for Sprint 13 meeting notes...")
    client = _client()
    ...
    if saved != OUT_PATH:
        print(
            ...
            "Close sprint13_meeting_notes.docx and rename/copy the new file, or rerun."
        )
    else:
        print(f"Wrote {saved}")
    return 0
