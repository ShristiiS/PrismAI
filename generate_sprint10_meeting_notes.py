...
# User referenced "sprint_10-13_Meeting_Notes.md"; in this repo the file is Sprint_10-13_Meeting_Notes.md.
SOURCE_MARKDOWN = DATASET_DIR / "Sprint_10-13_Meeting_Notes.md"
WORD_OUT_DIR = DATASET_DIR / "generated" / "word"
OUT_PATH = WORD_OUT_DIR / "sprint10_meeting_notes.docx"
...
def _extract_sprint10_meeting_blueprint(full_md: str) -> str:
    start_tag = "# MEETING NOTE 1 — SPRINT 10"
    start = full_md.find(start_tag)
    if start == -1:
        raise GenerationError(f"Could not find {start_tag!r} in {SOURCE_MARKDOWN.name}")

    end_tag = "# MEETING NOTE 2 — SPRINT 11"
...
def _call_meeting_json(client: OpenAI, *, blueprint: str) -> dict[str, Any]:
    user = (
        "Return ONLY valid JSON (no markdown, no code fences) for the Sprint 10 weekly meeting notes Word document.\n"
    ...
    for attempt in range(max_attempts):
        try:
            resp = client.chat.completions.create(
                ...
                messages=messages,
                temperature=0.35,
                # Avoid provider defaulting to a too-high max_tokens.
                max_tokens=12000,
...
def _write_docx(spec: dict[str, Any], out_path: Path) -> Path:
    ...
    try:
        ...
        fb = out_path.with_name("sprint10_meeting_notes_ready.docx")
...
def main() -> int:
    ...
    blueprint = _extract_sprint10_meeting_blueprint(md)

    print(f"Model: {MODEL}")
    print("Calling API for Sprint 10 meeting notes...")
    client = _client()
    ...
    if saved != OUT_PATH:
        print(
            ...
            "Close sprint10_meeting_notes.docx and rename/copy the new file, or rerun."
        )
    else:
        print(f"Wrote {saved}")
    return 0
