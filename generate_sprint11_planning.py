...
OUT_PATH = DATASET_DIR / "generated" / "word" / "sprint11_planning.docx"
...
def _extract_sprint11_planning_blueprint(full_md: str) -> str:
    start_tag = "# SPRINT 11 PLANNING DOCUMENT"
    end_tag = "# SPRINT 11 RETROSPECTIVE"
    ...
    if end == -1:
        raise GenerationError(f"Could not find {end_tag!r} after Sprint 11 planning section")
...
def _write_docx(data: dict[str, Any], out_path: Path) -> Path:
    ...
    try:
        ...
        fb = out_path.with_name("sprint11_planning_ready.docx")
...
def main() -> int:
    ...
    blueprint = _extract_sprint11_planning_blueprint(full_md)

    user = (
        "Return ONLY valid JSON (no markdown, no code fences) for a Sprint 11 planning Word document. "
        ...
        "{\n"
        '  "title": "string (e.g. Sprint 11 Planning — Nutrivana)",\n'
    ...
    print(f"Model: {MODEL}")
    print("Calling API for Sprint 11 planning document...")
    client = _client()
    ...
    if saved != OUT_PATH:
        print(
            ...
            "Close sprint11_planning.docx and copy/rename, or rerun."
        )
    else:
        print(f"Wrote {saved}")
    return 0
