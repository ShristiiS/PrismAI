...
# User referenced sprint10_13_planning_retro_definitions.md; in this repo the file is Sprint_10-13_Planning.md.
SOURCE_MARKDOWN = DATASET_DIR / "Sprint_10-13_Planning.md"
OUT_PATH = DATASET_DIR / "generated" / "excel" / "sprint12_retro.xlsx"
...
def _extract_sprint12_retro_blueprint(full_md: str) -> str:
    start_tag = "# SPRINT 12 RETROSPECTIVE"
    start = full_md.find(start_tag)
    if start == -1:
        raise GenerationError(f"Could not find {start_tag!r} in {SOURCE_MARKDOWN.name}")
    end_tag = "# SPRINT 13 PLANNING DOCUMENT"
...
def _call_retro_json(client: OpenAI, *, blueprint: str) -> dict[str, Any]:
    user = (
        "Return ONLY valid JSON (no markdown, no code fences) for the Sprint 12 retrospective.\n\n"
        "JSON schema:\n"
        "{\n"
...
def _write_workbook(spec: dict[str, Any], out_path: Path) -> Path:
    ...
    try:
        ...
        fb = out_path.with_name("sprint12_retro_ready.xlsx")
...
def main() -> int:
    ...
    blueprint = _extract_sprint12_retro_blueprint(full_md)

    print(f"Model: {MODEL}")
    print("Calling API for Sprint 12 retrospective...")
    client = _client()
    ...
    if saved != OUT_PATH:
        print(
            ...
            "Close sprint12_retro.xlsx and rename/copy the new file, or rerun."
        )
    else:
        print(f"Wrote {saved}")
    return 0
