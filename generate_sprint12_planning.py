...
OUT_PATH = DATASET_DIR / "generated" / "word" / "sprint12_planning.docx"
...
def _extract_sprint12_planning_blueprint(full_md: str) -> str:
    start_tag = "# SPRINT 12 PLANNING DOCUMENT"
    end_tag = "# SPRINT 12 RETROSPECTIVE"
    ...
    if end == -1:
        raise GenerationError(f"Could not find {end_tag!r} after Sprint 12 planning section")
...
def _write_docx(data: dict[str, Any], out_path: Path) -> Path:
    ...
    try:
        ...
        fb = out_path.with_name("sprint12_planning_ready.docx")
...
def main() -> int:
    ...
    blueprint = _extract_sprint12_planning_blueprint(full_md)

    user = (
        "Return ONLY valid JSON (no markdown, no code fences) for a Sprint 12 planning Word document. "
        ...
        "{\n"
        '  "title": "string (e.g. Sprint 12 Planning — Nutrivana)",\n'
    ...
    print(f"Model: {MODEL}")
    print("Calling API for Sprint 12 planning document...")
    client = _client()
    ...
    if saved != OUT_PATH:
        print(
            ...
            "Close sprint12_planning.docx and copy/rename, or rerun."
        )
    else:
        print(f"Wrote {saved}")
    return 0
