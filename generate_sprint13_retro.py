...
# User referenced "sprint10_13_planningmd"; in this repo the file is Sprint_10-13_Planning.md.
SOURCE_MARKDOWN = DATASET_DIR / "Sprint_10-13_Planning.md"
OUT_PATH = DATASET_DIR / "generated" / "excel" / "sprint13_retro.xlsx"
...
def _extract_sprint13_retro_blueprint(full_md: str) -> str:
    start_tag = "# SPRINT 13 RETROSPECTIVE"
    start = full_md.find(start_tag)
    if start == -1:
        raise GenerationError(f"Could not find {start_tag!r} in {SOURCE_MARKDOWN.name}")
    # Sprint 13 is the last sprint in this combined blueprint; stop at EOF.
...
def _validate_narrative_bullets(items: Any, field: str, *, min_items: int, max_items: int) -> None:
    ...
    for i, item in enumerate(items):
        ...
        if not _TICKET_ID_RE.search(t):
            raise ValueError(f"{field}[{i}] must reference at least one ticket ID")
...
def _call_retro_json(client: OpenAI, *, blueprint: str) -> dict[str, Any]:
    user = (
        "Return ONLY valid JSON (no markdown, no code fences) for the Sprint 13 retrospective.\n\n"
        "JSON schema:\n"
        "{\n"
    ...
    for attempt in range(max_attempts):
        try:
            resp = client.chat.completions.create(
                ...
                messages=messages,
                temperature=0.35,
                # Some providers default to very high max_tokens; cap to avoid credit/token errors.
                max_tokens=12000,
...
def _write_workbook(spec: dict[str, Any], out_path: Path) -> Path:
    ...
    try:
        ...
        fb = out_path.with_name("sprint13_retro_ready.xlsx")
...
def main() -> int:
    ...
    blueprint = _extract_sprint13_retro_blueprint(full_md)

    print(f"Model: {MODEL}")
    print("Calling API for Sprint 13 retrospective...")
    client = _client()
    ...
    if saved != OUT_PATH:
        print(
            ...
            "Close sprint13_retro.xlsx and rename/copy the new file, or rerun."
        )
    else:
        print(f"Wrote {saved}")
    return 0
