...
# User referenced "sprint_10-13_Meeting.md"; in this repo the file is Sprint_10-13_Meeting_Notes.md.
SOURCE_MARKDOWN = DATASET_DIR / "Sprint_10-13_Meeting_Notes.md"
WORD_OUT_DIR = DATASET_DIR / "generated" / "word"
OUT_PATH = WORD_OUT_DIR / "sprint12_meeting_notes.docx"
...
def _extract_sprint12_meeting_blueprint(full_md: str) -> str:
    start_tag = "# MEETING NOTE 3 — SPRINT 12"
    start = full_md.find(start_tag)
    if start == -1:
        raise GenerationError(f"Could not find {start_tag!r} in {SOURCE_MARKDOWN.name}")

    end_tag = "# MEETING NOTE 4 — SPRINT 13"
...
def _call_meeting_json(client: OpenAI, *, blueprint: str) -> dict[str, Any]:
    user = (
        "Return ONLY valid JSON (no markdown, no code fences) for the Sprint 12 weekly meeting notes Word document.\n"
        ...
        "(1) simplified onboarding user test results (84% vs 61%, 3 previous abandoners all completed), "
        "(2) ICMR vs WHO prenatal values decision (35mg iron vs 27mg, ICMR confirmed), "
        "(3) PRE-003 carry-over plan (Sprint 12 scope complete, Sprint 13 scope documented, June 26 deployment deadline).\n\n"
        "JSON schema:\n"
        "{\n"
        '  "document_title": "string (match blueprint document name tone)",\n'
        '  "meeting_date": "string (should be June 17, 2025)",\n'
        '  "sprint_line": "string (should read Sprint 12 (June 17–June 30, 2025) if present in blueprint)",\n'
        '  "attendees": "comma-separated names",\n'
        '  "sprint_health_check": "string multi-paragraph narrative",\n'
        '  "key_discussion_1": {"title": "Simplified onboarding user test results", "notes": "long live notes with who said what"},\n'
        '  "key_discussion_2": {"title": "ICMR vs WHO prenatal values decision", "notes": "..."},\n'
        '  "key_discussion_3": {"title": "PRE-003 carry-over plan", "notes": "..."},\n'
...
def _write_docx(spec: dict[str, Any], out_path: Path) -> Path:
    ...
    try:
        ...
        fb = out_path.with_name("sprint12_meeting_notes_ready.docx")
...
def main() -> int:
    ...
    blueprint = _extract_sprint12_meeting_blueprint(md)

    print(f"Model: {MODEL}")
    print("Calling API for Sprint 12 meeting notes...")
    client = _client()
    ...
    if saved != OUT_PATH:
        print(
            ...
            "Close sprint12_meeting_notes.docx and rename/copy the new file, or rerun."
        )
    else:
        print(f"Wrote {saved}")
    return 0
