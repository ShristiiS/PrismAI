...
OUT_PATH = DATASET_DIR / "generated" / "word" / "sprint10_planning.docx"
...
def _extract_sprint10_planning_blueprint(full_md: str) -> str:
    start_tag = "# SPRINT 10 PLANNING DOCUMENT"
    end_tag = "# SPRINT 10 RETROSPECTIVE"
    ...
    if end == -1:
        raise GenerationError(f"Could not find {end_tag!r} after Sprint 10 planning section")
...
def _write_docx(data: dict[str, Any], out_path: Path) -> Path:
    ...
    try:
        ...
        fb = out_path.with_name("sprint10_planning_ready.docx")
...
def main() -> int:
    ...
    blueprint = _extract_sprint10_planning_blueprint(full_md)

    user = (
        "Return ONLY valid JSON (no markdown, no code fences) for a Sprint 10 planning Word document. "
        ...
        "{\n"
        '  "title": "string (e.g. Sprint 10 Planning — Nutrivana)",\n'
    ...
    print(f"Model: {MODEL}")
    print("Calling API for Sprint 10 planning document...")
    client = _client()
    ...
    if saved != OUT_PATH:
        print(
            ...
            "Close sprint10_planning.docx and copy/rename, or rerun."
        )
    else:
        print(f"Wrote {saved}")
    return 0
