...
OUT_PATH = DATASET_DIR / "generated" / "jira" / "sprint1_jira.xlsx"
BLUEPRINT_PATH = DATASET_DIR / "Sprint1_Ticket_Definitions.md"
...
# Per-ticket overrides when the product owner wants different minimums than **Tier:** in the blueprint.
...
def _write_workbook(rows_tickets: list[list[Any]], rows_comments: list[list[Any]]) -> Path:
    ...
    try:
        ...
        fallback = OUT_PATH.with_name("sprint1_jira_ready.xlsx")
...
def main() -> int:
    ...
    if len(blocks) != 8:
        raise RuntimeError(f"Expected 8 ticket blocks, found {len(blocks)}")
    ...
    for block in blocks:
        ...
        rows_tickets.append(
            [
                ...
                str(data.get("sprint", "Sprint 1")).strip(),
    ...
    if saved != OUT_PATH:
        print(
            ...
            "Close sprint1_jira.xlsx and rename/copy the new file, or rerun this script."
        )
    else:
        print(f"\nWrote {saved}")
    return 0
